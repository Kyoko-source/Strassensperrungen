...
.card { border-radius: 18px; padding: 14px 16px; background: linear-gradient(135deg, rgba(13,110,253,0.08), rgba(110,66,193,0.08));
...
k1.markdown(f"""<div class="card"><div class="card-title">Pins gesamt</div><div class="card-value">{total}</div><div class="muted">Aktueller Stand</div></div>""", unsafe_allow_html=True)
k2.markdown(f"""<div class="card"><div class="card-title">Letzter Pin</div><div class="card-value">{("ID " + str(last_pin["id"])) if last_pin else "—"}</div><div class="muted">{last_pin["created_at"] if last_pin else "Noch keiner gesetzt"}</div></div>""", unsafe_allow_html=True)
k3.markdown(f"""<div class="card"><div class="card-title">Auswahl</div><div class="card-value">{("ID " + str(st.session_state.selected_pin_id)) if st.session_state.selected_pin_id else "—"}</div><div class="muted">Klick auf Marker oder Auswahl</div></div>""", unsafe_allow_html=True)
st.write("")
...
def build_map():
    ...
    for p in st.session_state.pins:
        ...
        folium.Marker(
            ...
            draggable=True,  # optisch; Speicherung Drag nicht zuverlässig
            icon=folium.Icon(color=p["color"])
        ).add_to(m)

    return m

def handle_map_events(res):
    # click on map -> create pin
...
# -----------------------------
# Main UI (fullscreen toggle)
# -----------------------------
if fullscreen:
    st.subheader("🗺️ Karte (Vollbild)")
    st.caption("Vollbildmodus innerhalb der App: Karte nutzt die gesamte Breite. Sidebar bleibt für Steuerung sichtbar.")
    m = build_map()
    res = st_folium(m, height=860, use_container_width=True)
    handle_map_events(res)
    ...
    if st.session_state.pins:
        ...
        df.rename(columns={"id": "ID", "type": "Typ", "name": "Name", "created_at": "Zeitstempel"}, inplace=True)
    ...
    with left:
        ...
        st.caption("Klick auf die Karte setzt einen Pin. Marker anklicken → Details rechts.")
        m = build_map()
